                            ...
                            <table class="table table-bordered" id="table2excel">
                                ...
                                @foreach($payments as $payment)
                                    <tr>
                                        ...
                                        <td class="text-nowrap">{{ $payment->receipt_no }}</td>
                                        <td class="text-nowrap">{{ date('d-m-Y',strtotime($payment->receipt_date)) }}</td>
                                        <td class="text-nowrap">{{ $payment->Contact->membership_no }}</td>
                                        <td class="text-nowrap">{{ $payment->Contact->contact_name }}</td>
                                        <td class="text-nowrap">{{ $payment->SiteAllotment->Project->project_name }}</td>
                                        <td class="text-nowrap">{{ $payment->SiteAllotment->Category->category_name }}</td>
                                        @if($payment->SiteAllotment->site_id == null)
                                            <td></td>
                                        @endif
                                        @if($payment->SiteAllotment->site_id !=null)
                                            <td class="text-nowrap">{{ $payment->SiteAllotment->Site->site_no }}</td>
                                        @endif
                                        <td class="text-nowrap">{{ $payment->payment_mode }}</td>
                                        <td class="text-nowrap text-right" align="right">{{ number_format($payment->amount,2) }}</td>
                                    </tr>
                                    @php
                                ...
                                @endforeach
                                <tr>
                                    <td class="text-nowrap text-right" style="text-align: right" colspan="9">Total Payment</td>
    ...
    <div id="filter-modal" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style="display: none;">
        <div class="modal-dialog"> 
            <div class="modal-content"> 
                <form action="{{ url('payment/payment_report') }}" method="get" autocomplete="off">
